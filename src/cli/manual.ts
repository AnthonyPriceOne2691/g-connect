...
async function cmdMap(ref: string, sheet?: string): Promise<void> {
  ...
  const snapshot = await (
    await client()
  ).getSpreadsheet(target.id, sheet === undefined ? {} : { sheet });
  ...
  console.log(
    `Шапка: ${map.headerRowReason}${map.headerRowConfident ? '' : '  ← выбрана неуверенно'}`,
  );
  console.log('\nКолонки:');
...
async function cmdUpsert(
  ...
  for (const question of outcome.questions) {
    ...
    if (question.candidates.length > 0)
      console.log(`    варианты: ${question.candidates.join(' | ')}`);
  ...
  for (const change of outcome.changes) {
    console.log(
      `  ${change.kind === 'addRow' ? '+' : '~'} ${change.a1} (${change.column}): ${String(change.before)} → ${String(change.after)}`,
    );
  }
  if (!write && outcome.changes.length > 0)
...
async function main(): Promise<void> {
  ...
  switch (command) {
    ...
    case 'map':
      if (rest[0] === undefined) {
        throw gcError('bad_request', { detail: 'Нужна ссылка: npm run gc -- map <ссылка> [лист]' });
      }
...
main().catch((error: unknown) => {
  if (isGcError(error)) {
    ...
    console.error(
      `  (${p.code}${p.cause === undefined ? '' : ` / ${p.cause}`}, ${p.correlationId})`,
    );
  } else {
