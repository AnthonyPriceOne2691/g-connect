...
export function normalizeValue(
  ...
  // Формула — не значение колонки, а вычисление. `=B2*3` в числовую колонку не «не число»:
  // Google с `USER_ENTERED` сам сделает из него формулу, и приводить его по типу колонки
  // нельзя. Нашёл пример HK9: до этой ветки нормализатор отвергал формулу как мусор, и
  // отдельная операция `setFormula` казалась нужной — она не нужна, нужна эта проверка.
  if (typeof raw === 'string' && raw.trimStart().startsWith('=')) {
    return {
      status: 'ok',
      value: raw.trim(),
      note: `«${raw.trim()}» записано формулой, а не текстом — как если бы её набрал человек`,
    };
  }
